        ...
        <div className="space-y-1">
          ...
          <div className="flex items-center gap-1.5">
            ...
            <span className="text-xs text-muted-foreground">
              {product.rating}/<span className="text-muted-foreground/60">5</span>
            </span>
          </div>

          {/* Price */}
          <div className="flex items-center gap-2">
            <span className="font-bold text-lg md:text-xl text-foreground">${product.price.toFixed(0)}</span>
            {product.originalPrice && (
              <>
                <span className="text-sm md:text-base text-muted-foreground line-through">
                  ${product.originalPrice.toFixed(0)}
                </span>
                <span className="px-2 py-0.5 bg-destructive/10 text-destructive text-[10px] md:text-xs font-medium rounded-full">
                  -{discountPercent}%
                </span>
              </>
            )}
