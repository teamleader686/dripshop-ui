  ...
  return (
    ...
    >
        ...
        <div className="star-rating">
          {[...Array(5)].map((_, i) => (
            <Star
              key={i}
              size={18}
              className={i < Math.floor(product.rating) ? 'star-filled' : 'text-muted-foreground/30'}
      ...
      <div className="flex items-center gap-3 mb-5">
        <span className="text-2xl md:text-[32px] font-bold">${product.price.toFixed(0)}</span>
        {product.originalPrice && (
          <>
            <span className="text-2xl md:text-[32px] text-muted-foreground line-through">
      ...
      <p className="text-muted-foreground text-sm leading-relaxed mb-6 pb-6 border-b border-border">
        {product.description}
      </p>
      ...
      <div className="mb-6 pb-6 border-b border-border">
        ...
        <div className="flex gap-3">
          {['bg-[hsl(30_15%_30%)]', 'bg-[hsl(150_30%_30%)]', 'bg-[hsl(220_30%_30%)]'].map((color, i) => (
            <button
              key={i}
              className={`w-9 h-9 rounded-full ${color} ${i === 0 ? 'ring-2 ring-foreground ring-offset-2' : ''}`}
            />
          ))}
        </div>
      </div>
          ...
          {['Small', 'Medium', 'Large', 'X-Large'].map((size, i) => (
            <button
              ...
              className={`px-5 py-2.5 rounded-full text-sm font-medium transition-colors ${
                i === 2 ? 'bg-primary text-primary-foreground' : 'bg-secondary text-muted-foreground hover:bg-primary hover:text-primary-foreground'
              }`}
            >
              {size}
            </button>
      ...
      <div className="hidden md:flex items-center gap-4 mb-6">
        <div className="flex items-center bg-secondary rounded-full">
          <button
            ...
            className="p-3.5 hover:bg-border/50 rounded-l-full transition-colors"
          >
            <Minus size={18} />
          </button>
          ...
          <button
            ...
            className="p-3.5 hover:bg-border/50 rounded-r-full transition-colors"
          >
            <Plus size={18} />
          </button>
        </div>

        <Button onClick={onAddToCart} size="lg" className="flex-1 btn-primary h-13 text-sm">
          Add to Cart
        </Button>
      </div>
      ...
      <div className="space-y-3 py-5 border-t border-border">
        ...
        <div className="flex items-center gap-3 text-sm">
          <RotateCcw size={18} className="text-muted-foreground flex-shrink-0" />
